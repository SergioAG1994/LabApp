-- Añade la etiqueta del análisis al listado seguro que consulta el personal.
drop function if exists public.list_staff_samples();

create function public.list_staff_samples()
returns table (
  order_id uuid,
  sample_id uuid,
  sample_code text,
...
begin
  ...
  select
    ...
    s.id as sample_id,
    s.sample_code,
    coalesce(nullif(btrim(s.analysis_label), ''), ap.name, ap.code, 'Sin análisis') as analysis_label,
    ao.received_at::date,
    ao.due_date::date,
