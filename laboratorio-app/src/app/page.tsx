...
import { missingStaffColumns, staffDisplayName, type LaboratoryStaff } from "@/lib/staff-attribution";
type Entry = {
    id: string;
    op: string;
    client: string;
...
export default function Home() {
    ...
    async function openSample(entry: Entry) {
        ...
        setSampledInput(entry.sampledAt || ""); setOrderRows([]); setWorksheetRevision(null); setLegacyWorksheet(false); setWorksheetLoading(true);
        ...
        const [currentResult, staffResult] = await Promise.all([
            supabase.from("worksheet_results").select(`${columns}, analyst_staff_id, released_by_staff_id, worksheet_revision, worksheet_sampled_at`).eq("sample_id", entry.sampleId).order("display_order"),
            supabase.from("laboratory_staff").select("id, full_name, initials, functions, active").order("full_name"),
        ]);
        const legacy = missingStaffColumns(currentResult.error);
        const result = legacy ? await supabase.from("worksheet_results").select(columns).eq("sample_id", entry.sampleId).order("display_order") : currentResult;
    ...
    async function persistWorksheet() {
        ...
        const rows = orderRows.map((row) => ({
            ...
            result_value: row.result_value?.trim() || null, analyst_reference: row.analyst_reference?.trim() || null,
