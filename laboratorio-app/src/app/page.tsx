...
export default function Home() {
    ...
    async function loadClientDirectory() {
        const { data, error } = await supabase.from("clients").select("client_number, name, contact_name, rfc, phone").eq("active", true).order("client_number");
    ...
    useEffect(() => {
        ...
        const header = document.createElement("thead"); const headerRow = document.createElement("tr"); ["ID cliente", "Cliente", "Contacto", "RFC", "Teléfono"].forEach((label) => { const cell = document.createElement("th"); cell.textContent = label; headerRow.append(cell); }); header.append(headerRow); table.append(header);
        const body = document.createElement("tbody"); clientDirectory.forEach((client) => { const row = document.createElement("tr"); [client.client_number.toString(), client.name, client.contact_name || dash, client.rfc || dash, client.phone || dash].forEach((value) => { const cell = document.createElement("td"); cell.textContent = value; row.append(cell); }); body.append(row); }); table.append(body); tableWrap.append(table); section.append(heading, tableWrap); form.append(section);
    ...
    function toggleEntry(id: string) { setSelectedEntryIds((current) => { const next = new Set(current); if (next.has(id))
        next.delete(id);
    else
