...
export default function Home() {
    ...
    const worksheetComplete = (!laboratorySampled || Boolean(sampledInput)) && orderRows.length > 0 && orderRows.every((row) => (uncertaintyIsNotApplicable(row.label) || (row.uncertainty !== null && row.uncertainty !== "")) && Boolean(row.result_value?.trim()) && Boolean(row.analyst_reference?.trim()) && Boolean(row.result_date) && Boolean(row.analyst_name?.trim()) && Boolean(row.released_by?.trim()));
